    ...
    public class UsersRepository
    {
        ...
        */
        ...
        public async Task<User> GetUserAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            return await _usersCollection.Find(Builders<User>.Filter.Eq(u => u.Email, email)).FirstOrDefaultAsync();
        }
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var resp = new UserResponse(newUser);
        ...
        public async Task<UserResponse> LoginUserAsync(User user, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                //  new UpdateOptions(...));
                var _filter = Builders<Session>.Filter.Eq(s => s.UserId, user.Email);

                var updateSessionUsr = await _sessionsCollection.UpdateOneAsync(_filter, Builders<Session>.Update
                    .Set(s => s.UserId, user.Email).Set(s => s.Jwt, user.AuthToken),
        ...
        public async Task<Session> GetUserSessionAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            var _filter = Builders<Session>.Filter.Eq(s => s.UserId, email);

            return await _sessionsCollection.Find(_filter).FirstOrDefaultAsync();
        }
            ...
            Dictionary<string, string> preferences, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                */
                ...
                //    new UpdateOptions(),
                //    cancellationToken)
                var _filter = Builders<User>.Filter.Eq(u => u.Email, email);
                
                updateResult = await _usersCollection.UpdateOneAsync(_filter, Builders<User>.Update.Set(u => u.Preferences, preferences));
