    ...
    public class MoviesRepository
    {
        ...
        public async Task<Movie> GetMovieAsync(string movieId, CancellationToken cancellationToken = default)
        {
            ...
            catch (Exception ex)
            {
                ...
                // throw.
                var ex1 = ex;
                if(ex.Message.Contains("is not a valid")) return null;
            ...
            )
        {
            ...
            //   .Project(...)
            //   .ToListAsync(cancellationToken);
            var _filter = Builders<Movie>.Filter.In("countries", countries);
            var _projection = Builders<Movie>.Projection
                                             .Include(m => m.Title)
                                             .Include(m => m.Id);
            ...
            return await _moviesCollection
                ...
                .Project<MovieByCountryProjection>(_projection)
                .ToListAsync(cancellationToken);
        }
            ...
            int page = 0, params string[] genres)
        {
            ...
            var _filter = Builders<Movie>.Filter.In("genres", genres);
